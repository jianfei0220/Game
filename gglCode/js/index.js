    ...
    $btn.click(function () {
		  layer.open({
		    ...
		    content:
	  			...
	  			+  '<input class="nameInput" name="nameInput" id="nameInput" type="text" value="" onkeyup="space(this)" placeholder="User name">'
	  			+  '</div>'
	  			+  '<div class="phoneDiv">'
	  			+  '<label for="phoneInput">Phone:</label>'
	  			+  '<input class="phoneInput" name="phoneInput" id="phoneInput" type="number" pattern="[0-9]*" value="" placeholder="Phone number">'
				  +  '</div>'
				  
	  			+  '<div class="verifyDiv" style="position:relative;">'
	  			+  '<label for="verifyCode">code:</label>'
	  			+  '<input class="verifyInput" name="verifyInput" id="verifyInput" maxlength="6" type="number" pattern="[0-9]*" placeholder="Verification code">'
	  			+  '<button type="button" onclick="getCode(this)" class="codeBtn" style="position:absolute;right:24px;top:14px;z-index:99999;">Get code</button>'
				  +  '</div>',
				yes: function(index){
			    phoneNumber = $('.phoneInput').val().replace(/0*(\d*)/,"$1");
					...
					if($('.phoneInput').val() ==''||userName =='' || authCode == ''){
						layer.open({type:'msg',skin: 'msg',content: 'Information cannot be blank！',time: 1});
					}else if(!isRightPhoneNumber(phoneNumber)){
						layer.open({type:'msg',skin: 'msg',content: 'Phone number format is incorrect！',time: 1});
					}else if(authCode.length != 6){
						layer.open({type:'msg',skin: 'msg',content: 'The verification code format is incorrect！',time: 1});
					}else if(!codeStatus){
						layer.open({type:'msg',skin: 'msg',content: 'Please get verification code！',time: 1});
					}
              ...
              function(data){
								...
								if(data.code == 20005){
									layer.open({type:'msg',skin: 'msg',content: 'Cannot repeatedly draw the lucky draw！',time: 1});
								}else if(data.code == 200){
									layer.open({type:'msg',skin: 'msg',content: 'ok！',time: 1});
